...
//
// `throw` landing supplied the mechanism and not the state. What is missing is
// a third answer for a binding — initialized, uninitialized, and the check on
// every read that distinguishes them. bronze has a Hole tag (`0xFFF7`) but it
// is spoken for: it is the empty
// pending-exception cell, so the dead zone needs a marker of its own rather
// than a reuse of that one.
//
// Two things are wanted with it. `ReferenceError` is not one of the three
// constructors the runtime builds, so it has to be added; and the
