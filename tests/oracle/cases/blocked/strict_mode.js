...
// answers instead.
//
// The mechanism this needed was `throw`, and `throw` has landed. What is
// missing now is the DIRECTIVE: ECMA-262 11.2.2 makes strictness a property of
...
// step 3 (OrdinarySetWithOwnDescriptor returns false when the property has no
// setter) combined with 13.15.2 PutValue step 6.d, which throws ONLY when the
// reference is strict. Making it throw unconditionally would be wrong for the
// sloppy code that is bronze's default, and `accessor_properties.expected` pins
// that sloppy answer.
"use strict";
...
// arrived: bronze has one answer for an unresolvable reference in every
// position, and it is the strict one — sloppy mode's implicit global was never
// available, because bronze has no global object to create a binding on. The
// case stays blocked on the two halves above and below, which are what a
// Directive Prologue would buy.
try {
  undeclared = 1;
  console.log("no throw");
} catch (e) {
