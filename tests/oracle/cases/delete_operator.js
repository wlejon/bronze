...
//
// An object's own properties are the nodes of a shape transition chain, and a
// chain is exactly the wrong structure to remove from the middle of: every
// shape below the removed node would have the wrong `slot_index`, and shapes
// are immortal and shared between objects, so the repair could not be local to
// the one object being deleted from. So the first successful delete moves that
// object — and only that object — to a private table it can be removed from.
//
...
//    than back to where it was. Insertion order is by last creation, and
//    only a delete can make that distinction visible.
