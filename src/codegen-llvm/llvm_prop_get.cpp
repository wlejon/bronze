...
llvm::Value* emitPropGet(llvm::IRBuilder<>& builder, const AbiFns& abi, const AbiGlobals& globals,
    ...
    if (keyStr == "length") {
        ...
        auto* arrLen = builder.CreateAlignedLoad(i32Ty, arrLenPtr, llvm::Align(4), "arr.len");
        tagArrayHeaderAccess(arrLen, ctx);
    ...
    if (join != nullptr) {
        llvm::BasicBlock* ownSlotEdge = ownSlotStepEnabled() ? staticGuard.hitBb : nullptr;
        join->fastBb = ownSlotEdge;
        join->doneBb = doneBb;
    }
    ...
    if (proof != nullptr) rejoinReceiverProof(builder, *proof, nullptr, doneBb);
    return result;
}
