...
// Give one function's IL skeleton the parameter and return types a `--pins`
// SIGNATURE entry declares for it (types/pins.h, the `param` and `return`
// forms). Returns false only after diagnosing.
//
// It runs after every proof has spoken — `applyProvenSignature` for a module
// function, the convention statements for a generator or an async factory — and
// it only ever moves a slot from `Dynamic` to `F64`. A pin that agrees with a
// proof is a no-op; one that disagrees cannot arise, because the only other
// thing an f64 slot could already be is the same f64.
//
// A refused POSITION is a hard error rather than a silent skip. A `param` entry
// names one function and one parameter of it, so it is not the "matched
...
bool Lowerer::applySignaturePins(const std::vector<ast::Param>& params, Span span,
    ...
    const size_t base = fn.firstSourceParam();
    for (size_t i = 0; i < params.size() && i + base < fn.params.size(); ++i) {
        const ast::Param& p = params[i];
        if (p.name.empty() || !pins_->paramPinned(fn.name, p.name)) continue;
        // The three positions the language may fill with a value the caller
        // never wrote, and there is no `undefined` in an f64. `directCallShapeFits`
        // states the same rule from the call site's side.
        const char* why = p.isRest       ? "a rest parameter is an array, not a number"
                          : p.pattern    ? "a destructuring pattern binds no single slot"
                          : p.defaultValue ? "a parameter with a default is `undefined` on a "
                                             "short call, which no f64 slot can hold"
                                           : nullptr;
        if (why != nullptr) {
            diags_.error(span, "pin 'param " + fn.name + "(" + p.name +
                                   "): number' cannot be honoured: " + std::string(why));
            return false;
        }
    ...
    if (pins_->returnPinned(fn.name)) {
        // A generator's factory returns the GENERATOR OBJECT and an async
        // function's the PROMISE, whatever the body returns — so a return pin
        // on either is a claim about the wrong value. Read off `fnFlags`, which
        // the caller has already set from the syntax; `isGenerator` is set
        // later, by `lowerFunctionBody`.
        if ((fn.fnFlags & (BRONZE_ABI_FN_FLAG_GENERATOR | BRONZE_ABI_FN_FLAG_ASYNC)) != 0) {
            diags_.error(span, "pin 'return " + fn.name +
                                   ": number' cannot be honoured: this function returns a "
                                   "generator object or a promise, not its body's value");
            return false;
        }
...
void Lowerer::recordMethodCallSite(const il::Instruction& inst, const ast::Expr& receiver,
    ...
    if (inference_ != nullptr) {
        const types::Type t = inference_->typeAt(&inferenceExpr(receiver));
        if (t.is(types::TypeKind::Object) && t.shapeClass() != types::kNoShapeClass) {
            if (const types::ClassLayout* cl = inference_->classLayouts.byShapeClass(t.shapeClass());
                cl != nullptr) {
                site.receiverClass = cl->name;
            }
        }
    }
    methodCallSites_[inst.icIndex] = std::move(site);
}

void Lowerer::recordClassMethod(const std::string& className, const std::string& superName,
...
void Lowerer::recordClassSuper(const std::string& className, const std::string& superName) {
...
//
// Runs once, with every function lowered, because that is the earliest point
// at which both halves exist. It only writes `directTarget`; a site it cannot
// resolve is the site it already was.
void Lowerer::resolveDirectMethodTargets() {
    ...
    if (disabled) return;
    if (methodCallSites_.empty() || classMethods_.empty()) return;
    ...
    auto resolve = [&](const std::string& className,
    ...
    for (auto& fn : ilModule_.functions) {
        for (auto& block : fn.blocks) {
            for (auto& inst : block.instructions) {
                ...
                const auto site = methodCallSites_.find(inst.icIndex);
                if (site == methodCallSites_.end() || site->second.receiverClass.empty()) continue;
                const uint32_t target = resolve(site->second.receiverClass, site->second.method);
                if (target >= ilModule_.functions.size()) continue;
                const il::Function& callee = ilModule_.functions[target];
...
void Lowerer::reportClassLayouts() {
    ...
    stats_->recordModuleLiteralAccessors(inference_->moduleLiterals.report());
    stats_->recordModuleLiteralInlines(inference_->moduleLiterals.inlineReport(),
                                       inlinedLiteralSites_);
