...
// The barrier compiled code calls after a store it makes itself (MIR's
// write_barrier, and the inline element store): the process's one definition
// of brass_gc_write_barrier, overriding brass's default. `obj` is the object
// stored into, as a Value or a raw address anywhere inside it; `val` the Value
// stored. It is the same barrier every C++ store takes (heap.h,
// gcWriteBarrier), on whichever of the thread's heaps holds the object.
extern "C" void brass_gc_write_barrier(uint64_t obj, uint64_t val) {
    const uintptr_t address = static_cast<uintptr_t>(obj & bronze::kPayloadMask);
    if (address == 0) return;
    bronze::gcWriteBarrier(reinterpret_cast<const void*>(address), bronze::Value(val));
}
