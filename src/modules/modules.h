...
// The module graph (docs/0023). A bronze build used to be one file; it is now
// an entry file plus everything it reaches through relative `import` and
// `export ... from` specifiers.
//
// Everything modules mean stops at this boundary. What comes out is ONE
// `ast::Module` — the graph flattened in evaluation order, with every
// non-entry file's module-level bindings renamed into a single namespace —
// so inference, lowering, the IL and the backend see the single-file program
// they saw before this existed (docs/0023 decision 1).
...
// A specifier as written, and the file it was written in, to the file it
// names. Relative only; everything else is a named error (docs/0023
// decision 3). False on a diagnosed error.
