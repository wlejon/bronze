...
// those write their own scopes. It is what sizes a block-argument SSA join — a
// name left out here is a variable frozen at its pre-loop value on a back edge.
//
...
// lowering and inference read — is defined in terms of it.
...
// functions excluded. The second half of "which bindings cannot live in SSA": a
// handler block is entered from an arbitrary point in the protected region, so
// nothing at IL-construction time knows what such a binding held there, and
// there is no block-argument list to write. Union it with `getCapturedNames`
// and lowering and inference have one answer to that question, which is the
// rule `queries.h` states.
//
