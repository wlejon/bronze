...
Status: implemented. Implements the prototype bullet of docs/0004
...
- reading a property JS does not define returns `undefined`, which is what
  JS says;
- reading a property JS DOES define and bronze has not built is a hard error
  naming it — `unsupported: Function.prototype.name is not implemented`;
- **writing** any property other than `prototype` is a hard error naming
  itself, rather than being dropped on the floor.

The first two are one distinction, and it is the whole of decision 3 in
docs/0011. `undefined` for `name`, `length`, `call`, `apply` or `bind` is a
claim about the language that is false, and the program then feature-tests it
away or calls it and dies naming bits — `[1].push` produced a call error
naming the bit pattern `fff6000000000000` rather than `push`. The same tables
cover `Array.prototype`, `String.prototype`, `%TypedArray%.prototype` and
`ArrayBuffer.prototype`.

Plain objects are deliberately left out: `getProp` cannot distinguish
"absent" from "present and holding `undefined`", so diagnosing
`Object.prototype` members there could hard-error on valid code, and a false
error is worse than a documented absence.
...
- `prototype chain too deep (a cycle?)`, so a cycle aborts rather than
  hangs
- `unsupported: Function.prototype.<name> is not implemented` (decision 5),
  and the same message for `Array.prototype`, `String.prototype`,
  `Float32Array.prototype` and `ArrayBuffer.prototype`
